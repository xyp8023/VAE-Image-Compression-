...
checkpoint_dir = 'train_crop1e-2'  # default train
last_step = 2000  # defauilt 1000000
Lambda = 0.01
for j in range(8):
	for i in range(16):
	...
	# train command
		train_command = 'python bls2017_2.py train --num_filters ' + str(num_filters)+ ' --batchsize '+str(batchsize) + ' --patchsize '\
						+ str(patchsize) + ' -v --data_glob ' +str(data_glob) + ' --checkpoint_dir ' + str(checkpoint_dir)+' --last_step '+ str(last_step*(16*i+j)) +' --lambda '+ str(Lambda)
