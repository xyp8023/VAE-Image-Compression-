...
def analysis_transform(tensor, num_filters):
  ...
  with tf.variable_scope("analysis"):
    ...
    return tensor
def analysis_transform_ga(tensor, num_filters, M):
  """Builds the analysis transform."""
  ...
  with tf.variable_scope("analysis"):
    ...
    return tensor
def analysis_transform_ha(tensor, num_filters):
  """Builds the analysis transform."""
  ...
  with tf.variable_scope("analysis"):
    with tf.variable_scope("layer_0"):
      layer = tfc.SignalConv2D(
          ...
          use_bias=True, activation='rule')
      tensor = layer(tf.math.abs(tensor))
...
def synthesis_transform(tensor, num_filters):
  ...
  with tf.variable_scope("synthesis"):
    ...
    return tensor
def synthesis_transform_gs(tensor, num_filters):
  """Builds the synthesis transform."""
  ...
  with tf.variable_scope("synthesis"):
    ...
    return tensor
def synthesis_transform_hs(tensor, num_filters, M):
  """Builds the synthesis transform."""
  ...
  with tf.variable_scope("synthesis"):
    ...
    with tf.variable_scope("layer_2"):
      layer = tfc.SignalConv2D(
          M, (3, 3), corr=False, strides_up=1, padding="same_zeros",
          use_bias=True, activation='rule')
      tensor = layer(tensor)
...
def train():
  ...
  y = analysis_transform_ga(x, args.num_filters, args.M)
  z = analysis_transform_ha(y, args.num_filtes)
  entropy_bottleneck = tfc.EntropyBottleneck()
  y_tilde, likelihoods = entropy_bottleneck(y, training=True)
  entropy_bottleneck1 = tfc.EntropyBottleneck()
  z_tilde, z_likelihoods = entropy_bottleneck1(z, training=True)
  # x_tilde = synthesis_transform(y_tilde, args.num_filters)
  sigma_tilde=synthesis_transform_hs(z_tilde, args.num_filters, args.M)
  x_tilde = synthesis_transform_gs(y_tilde, args.num_filters)
